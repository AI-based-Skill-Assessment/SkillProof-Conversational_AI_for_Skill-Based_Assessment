...
async def create_document_result(
    ...
    try:
        vp = VerificationPath(doc_in.verification_path) if doc_in.verification_path else VerificationPath.none
    except ValueError:
        vp = VerificationPath.falconebiz_registry if "falconebiz" in str(doc_in.verification_path) else VerificationPath.none
...
async def update_document_result(
    ...
    if doc:
        ...
        if verification_path is not None:
            try:
                doc.verification_path = VerificationPath(verification_path)
            except ValueError:
                doc.verification_path = VerificationPath.falconebiz_registry if "falconebiz" in str(verification_path) else VerificationPath.none
